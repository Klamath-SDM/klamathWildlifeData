...
Use the same bounding boxes used in the dynamic surface water elevation to assign locations to the eBird/BIRDBASE dataset. 
...
```{r}
bird_by_refuge <- ebd_birdbase_with_waterbirds |>
  mutate(
    area = case_when(
...
bird_by_refuge |> glimpse()
```

```{r}
pts_sf <- bird_by_refuge |>
...
ggplot(bird_compare |> 
         filter(area != "Klamath River"), aes(x = year, y = count, color = dataset)) +
  geom_line(linewidth = 1) +
  geom_point(size = 2) +
...
#        aes(x = year, y = dataset, fill = count)) +
#   geom_tile() +
#   facet_grid(common_name ~ area) +
#   theme_minimal() +
#   labs(
#     title = "Heatmap of Yearly Totals by Species, Area, and Dataset",
#     x = "Year",
#     y = "Area"
#   )
...
ggplot(bird_compare |> 
  ...
  labs(
    title = "Comparison of Waterbird Counts Across all Areas: RTR/USGS vs eBird",
    x = "Year",
    y = "Count",
    color = "Dataset"
  )
...
```{r}
waterbird_guild <- bird_by_refuge |> 
